...
const Login = () => {
    ...
    const onSubmit = async (data:LoginSchemaType) =>{
        ...
        try{
            ...
            // ensure expected token shape exists (optional guard)
            // if (!authPayload.backendTokens?.accessToken) { /* handle accordingly */ }

            dispatch({ type: "ADD_AUTH", payload: authPayload })
            form.reset()
                ...
                <div className='absolute rounded-t-3xl bottom-0 lg:relative w-full lg:w-[40%] pt-4 px-4 bg-white lg:px-0 lg:h-full  lg:mt-0 flex flex-col justify-end lg:justify-center gap-4 pb-4 lg:pb-0'>
                    <Form {...form}>
                        <form className='md:w-full xl:w-[80%]' onSubmit={form.handleSubmit(onSubmit)}>
                            ...
                            <button className='rounded-full flex items-center justify-center bg-blue-500 w-full text-white py-2 hover:bg-blue-300' disabled={isPending}> 
                                {!isPending && "Login"}
