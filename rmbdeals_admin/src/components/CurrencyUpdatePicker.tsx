...
const CurrencyPicker = ({ value: valueProp, onChange }: Props) => {
    const [open, setOpen] = useState(false)
    const [value, setValue] = useState<string>(valueProp ?? "GHS")
    ...
    // notify parent only when uncontrolled (so parent-controlled updates don't cause extra callbacks)
    useEffect(() => {
        if (valueProp === undefined && value) {
            onChange(value)
        }
                            ...
                            {currenciesQuery.data?.map((currency: Currency) => {
                                return (
                                    <CommandItem key={currency.currency} onSelect={() => {
                                        // always inform parent
                                        onChange(currency.currency)
                                        // update internal state only if uncontrolled
                                        if (valueProp === undefined) setValue(currency.currency)
                                        setOpen(prev => !prev)
                                    }}>
